...
def control_mouse(command):
	mapper = {
		'left': 'xdotool mousemove_relative -- -10 0', 
		'right': 'xdotool mousemove_relative --  10 0'
	}
	if command in mapper:
		subprocess.run(mapper[command], shell=True)
...
def grab_process():
	try:
		...
		if path.isfile('models/agent.h5'):
			model = tf.keras.models.load_model('models/agent.h5')
			...
			while keep_grabing:
				...
				if capture_status:
					...
					if output:
						...
						batch = np.ravel(adjacency_matrix)[None, :]
						prediction = np.argmax(model.predict(batch))
						predicted_command = mapper[sigma[prediction]]

						print(predicted_command)
						control_mouse(predicted_command)
